...
export default function UsageStats({ period: periodProp, setPeriod: setPeriodProp, hidePeriodSelector = false } = {}) {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6">
      ...
      {/* Combo efficiency summary - small inline card */}
      <ComboEfficiencyCard />
...
// Select reserves value="" for its own disabled placeholder, so "all combos" needs
// a sentinel of its own rather than an empty string.
const ALL_COMBOS = "__all__";

// Absolute local timestamp for the observed window. Deliberately not fmtTime(): a
// relative "2h ago" can't express a range's start and end.
...
// Combo routing efficiency: what the routed turns cost versus answering every one
// of them on the capable model. Spend only — it says nothing about answer quality.
function ComboEfficiencyCard() {
  const [period, setPeriod] = useState("today");
  const [combo, setCombo] = useState(ALL_COMBOS);
  // Results are stored with the selection they were fetched for, so "loading" is
  // derived rather than set from inside the effect (which would cascade renders).
  const [result, setResult] = useState({ key: null, payload: null, error: null });

  const key = `${period}|${combo}`;

  useEffect(() => {
    let cancelled = false;
    const [p, c] = key.split("|");
    const qs = new URLSearchParams({ period: p });
    if (c && c !== ALL_COMBOS) qs.set("combo", c);
    fetch(`/api/usage/combo-efficiency?${qs}`)
      .then(async (r) => {
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        return r.json();
      })
      .then((payload) => { if (!cancelled) setResult({ key, payload, error: null }); })
      .catch((e) => { if (!cancelled) setResult({ key, payload: null, error: e.message || "request failed" }); });
    ...
    <Card className="flex min-w-0 flex-col gap-4" padding="sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <h3 className="font-semibold">Combo Routing Efficiency</h3>
          <p className="mt-0.5 text-sm text-text-muted">
            Spend for the turns that ran, versus answering all of them on the capable model.
          </p>
        </div>
...
function ComboEfficiencyBlock({ data: c }) {
  const money = (n) => `$${(n || 0).toFixed(2)}`;

  return (
    <div className="flex min-w-0 flex-col gap-3 border-t border-border pt-4 first:border-t-0 first:pt-0">
      <div className="flex flex-wrap items-center gap-2">
        <code className="font-mono text-sm font-medium">{c.combo}</code>
        <Badge variant="default">{c.strategy}</Badge>
        <span className="text-sm text-text-muted">
          {fmt(c.turns)} turns · {fmtWindow(c.window?.from)} → {fmtWindow(c.window?.to)}
        </span>
      </div>
      ...
      <div className="min-w-0 overflow-x-auto">
        <table className="w-full min-w-[520px] text-left text-sm">
          ...
          <tbody className="divide-y divide-border/50">
            {c.perModel.map((m) => (
              <tr key={`${m.provider}/${m.model}`} className="transition-colors hover:bg-bg-subtle">
                <td className="px-3 py-2">
                  <span className="font-mono">{m.provider}/{m.model}</span>
                  {m.isCapable && <span className="ml-2 text-xs font-medium text-primary">capable</span>}
                </td>
                <td className="px-3 py-2 text-right text-text-muted">{fmt(m.calls)}</td>
                <td className="px-3 py-2 text-right text-text-muted">{fmt(m.inTok)}</td>
                <td className="px-3 py-2 text-right text-text-muted">{fmt(m.outTok)}</td>
                <td className="px-3 py-2 text-right font-medium text-warning">{money(m.cost)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-lg bg-bg-subtle px-3 py-2">
          <span className="block text-xs uppercase text-text-muted">Actual (routed)</span>
          <span className="text-lg font-bold text-warning">{money(c.actualCost)}</span>
        </div>
        <div className="rounded-lg bg-bg-subtle px-3 py-2">
          <span className="block text-xs uppercase text-text-muted">Baseline (all capable)</span>
          <span className="text-lg font-bold">{money(c.baselineCost)}</span>
          <span className="mt-0.5 block truncate font-mono text-xs text-text-muted" title={c.capableModel}>
            {c.capableModel}
          </span>
        </div>
        <div className="rounded-lg bg-bg-subtle px-3 py-2">
          <span className="block text-xs uppercase text-text-muted">{c.saved >= 0 ? "Saved" : "Overspent"}</span>
          <span className={`text-lg font-bold ${c.saved >= 0 ? "text-success" : "text-error"}`}>
            {money(Math.abs(c.saved))}
          </span>
          <span className="mt-0.5 block text-xs text-text-muted">
            {c.saved >= 0 ? `${c.savedPct}% of baseline` : "routing cost more than capable alone"}
          </span>
        </div>
        <div className="rounded-lg bg-bg-subtle px-3 py-2">
          <span className="block text-xs uppercase text-text-muted">Capable-tier share</span>
          <span className="text-lg font-bold text-primary">{c.capableShare}%</span>
          <span className="mt-0.5 block text-xs text-text-muted">
            {fmt(c.capableCalls)} of {fmt(c.turns)} turns
          </span>
        </div>
      </div>

      <p className="text-xs text-text-muted">
        Cost is estimated from the pricing tables, not a provider invoice. It measures spend
        only — judge answer quality separately.
      </p>
    </div>
  );
}
